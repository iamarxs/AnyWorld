...
- [ ] **Choose the intended state-pane layout** - Original requirements reserve 25% for state; current UI merges it with the log. Decide whether to retain current design or restore the separate pane; AGENTS.md now records the discrepancy.
- [ ] **Confirm dice range and threshold rules** - Current range is 0..100; describe_roll(90) says perfect success but the prompt says above 90. Choose intentional probabilities, centralize thresholds and test boundaries.
